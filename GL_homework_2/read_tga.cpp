...
void write_short( int16 value, FILE * file )
{
  value = ttohs( value );
...
int tga_write_raw( const char * file, int width, int height, 
           unsigned char * dat, unsigned int format ) {
...
//    case TGA_FLOAT_24:
    ...
    // write image spec.
    write_short( xorigin, tga );
    write_short( yorigin, tga );
    write_short( width, tga );
    write_short( height, tga );
...
int tga_write_rawZ( const char * file, int width, int height,  
           float * dat, unsigned int format, int r ) { 
    ...
    for( i = 0; i < size; i++ ) { 
        ...
        for( j = 0; j < format; j++ ) { 
            if(dat[i]>1 || dat[i]<0) 
             ...
             { 
            if(!r) 
            pixbuf |= (((long)(0xFF*(((double)dat[i]-zmin)/(zmax-zmin))))&0xFF) << (8 * j); 
            else 
            if(r) 
             pixbuf |= (((long)(0xFF*(((double)dat[i]))))&0xFF) << (8 * j); 
             } 
        } 
        ...
        case TGA_TRUECOLOR_24: 
            ...
            if(r) 
            { 
                //pixbuf =(long)(0xFFFFFF*((dat[i]-zmin)/(zmax-zmin))); 
                //pixbuf = htonl( pixbuf ); 
            } 
        ...
        */ 
            ...
            { 
                if(r) 
                pixbuf=(long)(0xFFFFFFFF*((dat[i]-zmin)/(zmax-zmin))); 
            } 
            fwrite( &pixbuf, 4, 1, tga ); 
...
int writez(const char *f,int width, int height,float * dat,float zn,float _zf,float zdx,float zdy) 
{ 
          FILE * zf = fopen( f, "wb" ); 
          fwrite(&width,4,1,zf); 
          fwrite(&height,4,1,zf); 
          long z32=32; 
          fwrite(&z32,4,1,zf); 
          fwrite(&zn,4,1,zf); 
          fwrite(&_zf ,4,1,zf); 
    ...
    { 
        ...
        { 
         ...
         { 
             float z;// 
             z=clamp(clamp((1-(((double)dat[i])/(zmax))))+0.20f);//+zmin))); 
             //z=1-dat[1]; 
             /*if(z>1 || z<=0) z=0; 
             else 
             { 
          ...
          //fwrite(dat,width*height*4,1,zf); 
